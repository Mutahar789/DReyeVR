...
void FCarlaServer::FPimpl::BindActions()
{
  ...
  BIND_SYNC(set_python_control_active) << [this](
      cr::ActorId ActorId,
      bool bIsActive) -> R<void>
  {
    REQUIRE_CARLA_EPISODE();
    FCarlaActor* CarlaActor = Episode->FindCarlaActor(ActorId);
    if (!CarlaActor)
    {
      return RespondError("set_python_control_active", ECarlaServerResponse::ActorNotFound);
    }
    ECarlaServerResponse Response = CarlaActor->SetPythonControlActive(bIsActive);
    if (Response != ECarlaServerResponse::Success)
    {
      return RespondError("set_python_control_active", Response);
    }
    return R<void>::Success();
  };
